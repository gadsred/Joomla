...
if (empty($list['direction']))
{
	...
	{  
				...
				if($id->link_id == '0')
				{
					$id->link_id = $jcookie->get('link_id', null);
				}
		...
		$query->from('`#__mt_faq` AS a');
		$query->where('( a.link_id LIKE '.$id->link_id.' )');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.question LIKE '.$search.'  OR  a.answer LIKE '.$search.' )');
			}
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(JText::_("COM_FAQ_SEARCH_FILTER_DATE_FORMAT"), "warning");

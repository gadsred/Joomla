...
<form action="<?php echo JRoute::_('index.php?option=com_mtprice&view=selprices'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<div class="wbc_h2container"><span>Price for Sellers</span></div>
			...
			<div class="price_buyer_boxes faq_topleft_border_last pi_normal border_bottom_mobile lesspadding_img <?php echo $gray_bg; ?>">
				<?php if($item->state !=2){?>
					<?php if ($canEdit || $canDelete): ?>
							<?php if ($canEdit): ?>
								<a href="<?php echo JRoute::_('index.php?option=com_mtprice&task=priceform.edit&Itemid=224&ptype=s&ktype=3&id=' . $item->id); ?>">EDIT</a>
							<?php endif; ?>
							<?php if ($canDelete): ?>
	...
	<?php if ($canCreate): ?>
		...
		<a href="<?php echo JRoute::_('index.php?option=com_mtprice&task=priceform.edit&Itemid=224&ptype=s&ktype=3&id=0&state='.$filter_au_state); ?>" class="buttons_main blue_bg long_button_adjust">
...
<script type="text/javascript">
	...
	function deleteItem() {
		...
		if (confirm("<?php echo JText::_('COM_MTPRICE_DELETE_MESSAGE'); ?>")) {
			window.location.href = '<?php echo JRoute::_('index.php?option=com_mtprice&task=priceform.remove&id=', false, 2) ?>' + item_id;
		}
		else{

 ...
 */
class MtpriceModelselPrices extends JModelList
{
...
if (empty($list['direction']))
{
	...
	protected function getListQuery()
	{
		...
		$query->where('a.keypoints_type= "2" and a.price_type="s" and a.id!="0"');
...
if (!JFactory::getUser()->authorise('core.edit.state', 'com_mtprice'))
{
	 ...
	 function getExtra()
	{
		...
		// Join over the created by field 'user_id'
		$query->join('LEFT', '#__users AS user_id ON user_id.id = a.user_id');
		$query->where('a.keypoints_type= "1" and price_type="s"');
	...
	function getPrice()
	{
		...
		if($user->name)
		{
			...
			$query->where('a.price_type="s" and a.link_id='.$link_id);
